...
*Note*  You may need to copy the DML jars into the harvestar by executing the following commands:

 find ~/.m2/repository/pt/ist -name \*.jar | grep 2.0-cloudtm | grep SNAPSHOT.jar > libs_paths

 for i in `cat libs_paths`; do cp $i PATH_TO_THE_HARVESTAR/lib/cloud_tm/jars; done
